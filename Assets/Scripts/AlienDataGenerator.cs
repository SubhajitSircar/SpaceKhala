...
{
    public static AlienDataGenerator Instance { get; private set; }
    ...
    public AlienProfile GenerateNewProfile()
    {
        ...
        // 3. Generate Personality Dialogue
        ...
        // Mask the cargo name inside the dialogue string template
        profile.dialogue = selectedTemplate.Replace("{CARGO}", $"[{profile.cargoItem}]").Replace("{NAME}", profile.alienName);

        // 4. Generate Random Target Frequency & Phase (0.1 to 0.9)
